...
**Purpose**: Configure the database connection and collections.

**Key Components**:
1. **Environment variables**: `MONGODB_URI`, `MONGODB_DB_NAME`
2. **Users collection**: `users` documents containing the user fields used by the auth system
3. **Indexes**: unique index on `email` recommended
...
2. **Update Database**:
   - Ensure any existing users have valid `role` values.
   - Update any role allowlists/guards in your UI/pages as needed.
