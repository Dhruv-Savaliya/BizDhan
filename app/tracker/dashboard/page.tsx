...
export default function DashboardPage() {
  ...
  const growthRate = useMemo(() => {
    if (totals.expenses === 0 && totals.income === 0) return { value: 0, type: "neutral" as const };
    if (totals.expenses === 0) return { value: 100, type: "up" as const };
    const rate = ((totals.income - totals.expenses) / totals.expenses) * 100;
    return { value: Math.abs(Math.round(rate * 10) / 10), type: rate >= 0 ? "up" as const : "down" as const };
  }, [totals]);
  ...
  const activityFeed = useMemo((): ActivityEvent[] => {
    ...
    for (const item of income.slice(0, 5)) {
      events.push({
        id: item.id || (item as Record<string, unknown>)._id as string || `inc-${Math.random()}`,
        type: "income",
    ...
    for (const item of expenses.slice(0, 5)) {
      events.push({
        id: item.id || (item as Record<string, unknown>)._id as string || `exp-${Math.random()}`,
        type: "expense",
        title: "Expense added",
    ...
    for (const item of investments.slice(0, 3)) {
      events.push({
        id: item.id || (item as Record<string, unknown>)._id as string || `inv-${Math.random()}`,
        type: "investment",
